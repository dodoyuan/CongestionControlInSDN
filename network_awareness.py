...
class NetworkAwareness(app_manager.RyuApp):
            ...
            for dst in self.switches:
                if src == dst:
                    ...
                    if (src, dst) not in self.edges and (dst, src) not in self.edges:
                        self.edges[(src, dst)] = defaultdict(lambda: defaultdict(int))
    ...
    # 通过跳数确定最短路径
    def all_datapaths_path(self, graph, weight='weight'):
        """
            Creat all paths between datapaths.
        """
        ...
        for src in _graph.nodes():
            all_paths.setdefault(src, {src: [[src]]})
            for dst in _graph.nodes():
                if src == dst:
                    continue
                all_paths[src].setdefault(dst, [])
                all_paths[src][dst] = self.all_paths_src_dst(_graph, src, dst,
                                                             weight=weight)
        return all_paths
    ...
    def get_topology(self, ev):
        ...
        self.shortest_paths = self.all_datapaths_path(
